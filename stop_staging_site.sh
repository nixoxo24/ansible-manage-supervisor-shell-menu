...
function select_site_option_staging() {
  ...
  read SITE_SELECT
    ...
    1)
      ...
      ansible-playbook -i inventory supervisor_stop.yaml --limit datacenter | tee -a $SCRIPT_LOG_FILE #splunk-servers # ansible command to restart all servers
  ...
  case $SPECIFIC_staging in
    1)
      ...
      ansible-playbook -i inventory supervisor_stop.yaml --limit server1 | tee -a $SCRIPT_LOG_FILE #splunk1
  
    ;;

    2)
      echo "Splunk-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_stop.yaml --limit server2 | tee -a $SCRIPT_LOG_FILE #splunk2
