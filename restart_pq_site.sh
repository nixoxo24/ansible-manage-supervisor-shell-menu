...
function select_site_option_pq() {
  ...
  case $SPECIFIC_PQ in
    1)
      ...
      ansible-playbook -i inventory supervisor_restart.yaml --limit issuerse_bancnet1_pq
  
    ;;

    2)
      echo "Bancnet-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit issuerse_bancnet2_pq
      
    ;;

    3)
      echo "Visa-1" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit issuerse_visa1_pq

    ;;

    4)
      echo "Visa-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit issuerse_visa2_pq
    ;;

    5)
      echo "Smartpay-1" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit issuerse_smartpay1_pq
    ;;
    6)
      echo "Smartpay-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit issuerse_smartpay2_pq
    ;;
    7)
      echo "Card id-1" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit cardidse1_pq
    ;;
    8)
      echo "Card id-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit cardidse2_pq 

    ;;
    9)
      echo "Key exchange-1" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit key_exchange1_pq 
    ;;
    10)
      echo "Key exchange-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit  key_exchange2_pq 
    ;;
    11)
      echo "EFS Personalization-1" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit efs1_personalization_pq
    ;;
    12)
      echo "EFS Personalization-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit efs2_personalization_pq
    ;;
    13)
      echo "Pin verification-1" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit pin_verificationse1_pq
    ;;
    14)
      echo "Pin verification-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit pin_verificationse2_pq
    ;;
    15)
      echo "EFS Wrapper-1" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit efs_wrapper1_pq
    ;;
    16)
      echo "EFS Wrapper-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit efs_wrapper2_pq
    ;;
    17)
      echo "EFS Wrapper-3" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit efs_wrapper3_pq
    ;;
    18)
      echo "EFS Wrapper-4" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit efs_wrapper4_pq
    ;;
    19)
      echo "EFS Portal-1" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit efs_portal1_wrapper_pq
    ;;
    20)
      echo "EFS Portal-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit efs_portal2_wrapper_pq
    ;;
    21)
      echo "Bus Portal-1" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit busportalese1_pq
    ;;
    22)
      echo "Bus Portal-2" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit busportalese2_pq
    ;;
    23)
      echo "SFTP" | tee -a $SCRIPT_LOG_FILE
      ansible-playbook -i inventory supervisor_restart.yaml --limit sftp_pq
    ;;
